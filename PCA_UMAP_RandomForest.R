...
# select top genes
top_genes <- names(sort(gene_scores, decreasing = TRUE))[1:200]

#create a dataframe woth only the top 200 genes from log_top
...
rf_spec <- rand_forest(mode = "classification", engine = "ranger", trees = 500, mtry = tune(), min_n = tune())
